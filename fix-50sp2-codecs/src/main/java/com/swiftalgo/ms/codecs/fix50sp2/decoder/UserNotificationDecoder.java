...
import com.swiftalgo.ms.codecs.fix50sp2.builder.UserNotificationEncoder;
...
public class UserNotificationDecoder extends CommonDecoderImpl implements UsernameGrpDecoder, MessageDecoder
{
...

    {
        ...
        {
            REQUIRED_FIELDS.add(Constants.USER_STATUS);
        }
    }
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            GROUP_FIELDS.add(Constants.USER_STATUS);
    ...
    public boolean validate()
    {
        ...
        if (hasNoUsernamesGroupCounter)
        {
            {
                int count = 0;
                final UsernamesGroupIterator iterator = usernamesGroupIterator.iterator();
                for (final UsernamesGroupDecoder group : iterator)
                {
    ...
    public static final long MESSAGE_TYPE = 16963L;

    public static final String MESSAGE_TYPE_AS_STRING = "CB";
    ...
    private UsernamesGroupDecoder usernamesGroup = null;
    public UsernamesGroupDecoder usernamesGroup()
    {
        return usernamesGroup;
    }

    private int noUsernamesGroupCounter = MISSING_INT;
    ...
    public int noUsernamesGroupCounter()
    {
        if (!hasNoUsernamesGroupCounter)
        {
            throw new IllegalArgumentException("No value for optional field: NoUsernamesGroupCounter");
        }
    ...
    private UsernamesGroupIterator usernamesGroupIterator = new UsernamesGroupIterator(this);
    public UsernamesGroupIterator usernamesGroupIterator()
    {
        return usernamesGroupIterator.iterator();
    }
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.NO_USERNAMES_GROUP_COUNTER:
                ...
                {
                    usernamesGroup = new UsernamesGroupDecoder(trailer, messageFields);
                }
                UsernamesGroupDecoder usernamesGroupCurrent = usernamesGroup;
                position = endOfField + 1;
                final int noUsernamesGroupCounter = this.noUsernamesGroupCounter;
                for (int i = 0; i < noUsernamesGroupCounter && position < end; i++)
                {
                    if (usernamesGroupCurrent != null)
                    {
                        position += usernamesGroupCurrent.decode(buffer, position, end - position);
                        usernamesGroupCurrent = usernamesGroupCurrent.next();
                    }
                }
                if (CODEC_VALIDATION_ENABLED)
                {
                    ...
                    if (checkEqualsPosition != AsciiBuffer.UNKNOWN_INDEX)
                    {
                        ...
                        if (usernamesGroup.ALL_GROUP_FIELDS.contains(checkTag))
                        {
                            invalidTagId = tag;
                            rejectReason = 16;
                            return position;
    ...
    {
        for (final UsernamesGroupDecoder usernamesGroupDecoder : usernamesGroupIterator.iterator())
        {
            usernamesGroupDecoder.reset();
            if (usernamesGroupDecoder.next() == null)
            {
                break;
            }
        }
        noUsernamesGroupCounter = MISSING_INT;
     ...
     * {@inheritDoc}
     */
    public UserNotificationEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((UserNotificationEncoder)encoder);
    }

    public UserNotificationEncoder toEncoder(final UserNotificationEncoder encoder)
    {
        encoder.reset();

        final UsernameGrpEncoder usernameGrp = encoder.usernameGrp();        if (hasNoUsernamesGroupCounter)
        {
            final int size = this.noUsernamesGroupCounter;
            UsernamesGroupDecoder usernamesGroup = this.usernamesGroup;
            UsernamesGroupEncoder usernamesGroupEncoder = usernameGrp.usernamesGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (usernamesGroup != null)
                {
                    usernamesGroup.toEncoder(usernamesGroupEncoder);
                    usernamesGroup = usernamesGroup.next();
