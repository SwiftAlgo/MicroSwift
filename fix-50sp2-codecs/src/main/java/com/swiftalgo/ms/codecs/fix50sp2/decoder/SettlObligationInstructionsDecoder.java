...
import com.swiftalgo.ms.codecs.fix50sp2.builder.SettlObligationInstructionsEncoder;
import static com.swiftalgo.ms.codecs.fix50sp2.builder.SettlObligationInstructionsEncoder.SettlObligGroupEncoder;
...
public interface SettlObligationInstructionsDecoder 
{



public class SettlObligGroupDecoder extends CommonDecoderImpl implements InstrumentDecoder, PartiesDecoder, SettlDetailsDecoder
{
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            GROUP_FIELDS.add(Constants.SETTL_OBLIG_ID);
            GROUP_FIELDS.add(Constants.SETTL_OBLIG_TRANS_TYPE);
            GROUP_FIELDS.add(Constants.SETTL_OBLIG_REF_ID);
            GROUP_FIELDS.add(Constants.CCY_AMT);
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            ALL_GROUP_FIELDS.add(Constants.SETTL_OBLIG_ID);
            ALL_GROUP_FIELDS.add(Constants.SETTL_OBLIG_TRANS_TYPE);
            ALL_GROUP_FIELDS.add(Constants.SETTL_OBLIG_REF_ID);
            ALL_GROUP_FIELDS.add(Constants.CCY_AMT);
    ...
    public boolean validate()
    {
        ...
        {
        if (CODEC_REJECT_UNKNOWN_ENUM_VALUE_ENABLED && !SettlObligTransType.isValid(settlObligTransType()))
        {
            invalidTagId = 1162;
            rejectReason = 5;
            return false;
    ...
    public SettlObligGroupDecoder(final TrailerDecoder trailer, final IntHashSet messageFields)
    {
        this.trailer = trailer;
        this.messageFields = messageFields;
    }

    private SettlObligGroupDecoder next = null;

    public SettlObligGroupDecoder next()
    {
        return next;
    }

    private IntHashSet seenFields = new IntHashSet(32);
 ...
 return NetGrossInd.NULL_VAL;
    ...
    private char[] settlObligID = new char[1];

    private boolean hasSettlObligID;

    public char[] settlObligID()
    {
        if (!hasSettlObligID)
        {
            throw new IllegalArgumentException("No value for optional field: SettlObligID");
        }
    ...
    {
        return hasSettlObligID;
    }


    private int settlObligIDOffset;

    private int settlObligIDLength;

    public int settlObligIDLength()
    {
        if (!hasSettlObligID)
        {
            throw new IllegalArgumentException("No value for optional field: SettlObligID");
        ...
        return settlObligIDLength;
    }

    public String settlObligIDAsString()
    {
        return hasSettlObligID ? new String(settlObligID, 0, settlObligIDLength) : null;
    }

    public void settlObligID(final AsciiSequenceView view)
    {
        if (!hasSettlObligID)
        {
            throw new IllegalArgumentException("No value for optional field: SettlObligID");
        ...
        view.wrap(buffer, settlObligIDOffset, settlObligIDLength);
    }


    private final CharArrayWrapper settlObligIDWrapper = new CharArrayWrapper();
    private char settlObligTransType = MISSING_CHAR;
    ...
    public char settlObligTransType()
    {
        if (!hasSettlObligTransType)
        {
            throw new IllegalArgumentException("No value for optional field: SettlObligTransType");
        }
    ...
    private final CharArrayWrapper settlObligTransTypeWrapper = new CharArrayWrapper();
    public SettlObligTransType settlObligTransTypeAsEnum()
    {
        if (!hasSettlObligTransType)
 return SettlObligTransType.NULL_VAL;
        return SettlObligTransType.decode(settlObligTransType);
    }

    private char[] settlObligRefID = new char[1];

    private boolean hasSettlObligRefID;

    public char[] settlObligRefID()
    {
        if (!hasSettlObligRefID)
        {
            throw new IllegalArgumentException("No value for optional field: SettlObligRefID");
        }
    ...
    {
        return hasSettlObligRefID;
    }


    private int settlObligRefIDOffset;

    private int settlObligRefIDLength;

    public int settlObligRefIDLength()
    {
        if (!hasSettlObligRefID)
        {
            throw new IllegalArgumentException("No value for optional field: SettlObligRefID");
        ...
        return settlObligRefIDLength;
    }

    public String settlObligRefIDAsString()
    {
        return hasSettlObligRefID ? new String(settlObligRefID, 0, settlObligRefIDLength) : null;
    }

    public void settlObligRefID(final AsciiSequenceView view)
    {
        if (!hasSettlObligRefID)
        {
            throw new IllegalArgumentException("No value for optional field: SettlObligRefID");
        ...
        view.wrap(buffer, settlObligRefIDOffset, settlObligRefIDLength);
    }


    private final CharArrayWrapper settlObligRefIDWrapper = new CharArrayWrapper();
    private DecimalFloat ccyAmt = DecimalFloat.newNaNValue();
    ...
    public DecimalFloat ccyAmt()
    {
        if (!hasCcyAmt)
        {
            throw new IllegalArgumentException("No value for optional field: CcyAmt");
        }
 ...
 return CPProgram.NULL_VAL;
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            if (!seenFields.add(tag))
            {
                if (next == null)
                {
                    next = new SettlObligGroupDecoder(trailer, messageFields);
                }
            ...
            case Constants.SETTL_OBLIG_ID:
                hasSettlObligID = true;
                settlObligID = buffer.getChars(settlObligID, valueOffset, valueLength);
                settlObligIDOffset = valueOffset;
                settlObligIDLength = valueLength;
                break;

            case Constants.SETTL_OBLIG_TRANS_TYPE:
                hasSettlObligTransType = true;
                settlObligTransType = buffer.getChar(valueOffset);
                break;

            case Constants.SETTL_OBLIG_REF_ID:
                hasSettlObligRefID = true;
                settlObligRefID = buffer.getChars(settlObligRefID, valueOffset, valueLength);
                settlObligRefIDOffset = valueOffset;
                settlObligRefIDLength = valueLength;
                break;

            case Constants.CCY_AMT:
                hasCcyAmt = true;
                ccyAmt = getFloat(buffer, ccyAmt, valueOffset, valueLength, 1157, CODEC_VALIDATION_ENABLED);
                break;
    ...
    public StringBuilder appendTo(final StringBuilder builder, final int level)
    {
        ...
        if (hasSettlObligID())
        {
            ...
            builder.append(this.settlObligID(), 0, settlObligIDLength());
            builder.append("\",\n");
        }
        ...
        if (hasSettlObligRefID())
        {
            ...
            builder.append(this.settlObligRefID(), 0, settlObligRefIDLength());
            builder.append("\",\n");
        }
     ...
     * {@inheritDoc}
     */
    public SettlObligationInstructionsEncoder.SettlObligGroupEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((SettlObligationInstructionsEncoder.SettlObligGroupEncoder)encoder);
    }

    public SettlObligationInstructionsEncoder.SettlObligGroupEncoder toEncoder(final SettlObligationInstructionsEncoder.SettlObligGroupEncoder encoder)
    {
        encoder.reset();
        if (hasNetGrossInd())
        {
        ...
        if (hasSettlObligID())
        {
            encoder.settlObligID(this.settlObligID(), 0, settlObligIDLength());
        }
        ...
        if (hasSettlObligRefID())
        {
            encoder.settlObligRefID(this.settlObligRefID(), 0, settlObligRefIDLength());
        }
    ...
    public class SettlObligGroupIterator implements Iterable<SettlObligGroupDecoder>, java.util.Iterator<SettlObligGroupDecoder>
    {
        private final SettlObligationInstructionsDecoder parent;
        private int remainder;
        private SettlObligGroupDecoder current;

        public SettlObligGroupIterator(final SettlObligationInstructionsDecoder parent)
        {
            this.parent = parent;
        }
        ...
        {
            remainder--;
            final SettlObligGroupDecoder value = current;
        ...
        {
            return parent.hasNoSettlObligGroupCounter() ? parent.noSettlObligGroupCounter() : 0;
        }

        public void reset()
        {
            remainder = numberFieldValue();
            current = parent.settlObligGroup();
        }

        public SettlObligGroupIterator iterator()
        {
            reset();
            return this;
        }
    ...
    public SettlObligGroupIterator settlObligGroupIterator();
    public int noSettlObligGroupCounter();
    public boolean hasNoSettlObligGroupCounter();
    public SettlObligGroupDecoder settlObligGroup();
