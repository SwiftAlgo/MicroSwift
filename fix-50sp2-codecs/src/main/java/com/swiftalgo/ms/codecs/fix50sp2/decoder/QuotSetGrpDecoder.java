...
public interface QuotSetGrpDecoder 
{



public class QuoteSetsGroupDecoder extends CommonDecoderImpl implements UnderlyingInstrumentDecoder, QuotEntryGrpDecoder
{
    ...
    public final IntHashSet GROUP_FIELDS = new IntHashSet(146);
 ...
 return UnderlyingFXRateCalc.NULL_VAL;
     ...
     * {@inheritDoc}
     */
    public QuotSetGrpEncoder.QuoteSetsGroupEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((QuotSetGrpEncoder.QuoteSetsGroupEncoder)encoder);
    }

    public QuotSetGrpEncoder.QuoteSetsGroupEncoder toEncoder(final QuotSetGrpEncoder.QuoteSetsGroupEncoder encoder)
    {
        encoder.reset();
        ...
        final QuotEntryGrpEncoder quotEntryGrp = encoder.quotEntryGrp();        if (hasNoQuoteEntriesGroupCounter)
        {
            final int size = this.noQuoteEntriesGroupCounter;
            QuoteEntriesGroupDecoder quoteEntriesGroup = this.quoteEntriesGroup;
            QuoteEntriesGroupEncoder quoteEntriesGroupEncoder = quotEntryGrp.quoteEntriesGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (quoteEntriesGroup != null)
                {
    ...
    {
        private final QuotSetGrpDecoder parent;
        private int remainder;
        private QuoteSetsGroupDecoder current;

        public QuoteSetsGroupIterator(final QuotSetGrpDecoder parent)
        {
            this.parent = parent;
        }
