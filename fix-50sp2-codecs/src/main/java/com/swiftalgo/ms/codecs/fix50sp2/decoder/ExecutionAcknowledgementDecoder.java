...
import com.swiftalgo.ms.codecs.fix50sp2.builder.ExecutionAcknowledgementEncoder;
...
public class ExecutionAcknowledgementDecoder extends CommonDecoderImpl implements InstrumentDecoder, UndInstrmtGrpDecoder, InstrmtLegGrpDecoder, OrderQtyDataDecoder, MessageDecoder
{
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            REQUIRED_FIELDS.add(Constants.EXEC_ACK_STATUS);
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            GROUP_FIELDS.add(Constants.EXEC_ACK_STATUS);
    ...
    public boolean validate()
    {
        ...
        if (CODEC_REJECT_UNKNOWN_ENUM_VALUE_ENABLED && !ExecAckStatus.isValid(execAckStatus()))
        {
            invalidTagId = 1036;
            rejectReason = 5;
            return false;
    ...
    public static final long MESSAGE_TYPE = 20034L;

    public static final String MESSAGE_TYPE_AS_STRING = "BN";

    public static final char[] MESSAGE_TYPE_CHARS = MESSAGE_TYPE_AS_STRING.toCharArray();

    public static final byte[] MESSAGE_TYPE_BYTES = MESSAGE_TYPE_AS_STRING.getBytes(US_ASCII);

    public final IntHashSet messageFields = new IntHashSet(614);

    {
        ...
        messageFields.add(Constants.EXEC_ACK_STATUS);
    ...
    private char execAckStatus = MISSING_CHAR;
    ...
    private final CharArrayWrapper execAckStatusWrapper = new CharArrayWrapper();
    public ExecAckStatus execAckStatusAsEnum()
    {
        return ExecAckStatus.decode(execAckStatus);
    }
    ...
    public char dKReason()
    {
        if (!hasDKReason)
        {
            throw new IllegalArgumentException("No value for optional field: DKReason");
        }
 ...
 return PriceType.NULL_VAL;
    ...
    {
        // Decode ExecutionAcknowledgement
        ...
        while (position < end)
        {
            ...
            case Constants.EXEC_ACK_STATUS:
                execAckStatus = buffer.getChar(valueOffset);
                break;
     ...
     * {@inheritDoc}
     */
    public ExecutionAcknowledgementEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((ExecutionAcknowledgementEncoder)encoder);
    }

    public ExecutionAcknowledgementEncoder toEncoder(final ExecutionAcknowledgementEncoder encoder)
    {
        encoder.reset();
