...
public interface StrmAsgnRptGrpDecoder 
{



public class AsgnReqsGroupDecoder extends CommonDecoderImpl implements PartiesDecoder, StrmAsgnRptInstrmtGrpDecoder
{
     ...
     * {@inheritDoc}
     */
    public StrmAsgnRptGrpEncoder.AsgnReqsGroupEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((StrmAsgnRptGrpEncoder.AsgnReqsGroupEncoder)encoder);
    }

    public StrmAsgnRptGrpEncoder.AsgnReqsGroupEncoder toEncoder(final StrmAsgnRptGrpEncoder.AsgnReqsGroupEncoder encoder)
    {
        encoder.reset();
        ...
        final StrmAsgnRptInstrmtGrpEncoder strmAsgnRptInstrmtGrp = encoder.strmAsgnRptInstrmtGrp();        if (hasNoRelatedSymGroupCounter)
        {
            final int size = this.noRelatedSymGroupCounter;
            RelatedSymGroupDecoder relatedSymGroup = this.relatedSymGroup;
            RelatedSymGroupEncoder relatedSymGroupEncoder = strmAsgnRptInstrmtGrp.relatedSymGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (relatedSymGroup != null)
                {
    ...
    {
        private final StrmAsgnRptGrpDecoder parent;
        private int remainder;
        private AsgnReqsGroupDecoder current;

        public AsgnReqsGroupIterator(final StrmAsgnRptGrpDecoder parent)
        {
            this.parent = parent;
        }
