...
import com.swiftalgo.ms.codecs.fix50sp2.builder.MultilegOrderCancelReplaceEncoder;
...
public class MultilegOrderCancelReplaceDecoder extends CommonDecoderImpl implements PartiesDecoder, PreAllocMlegGrpDecoder, DisplayInstructionDecoder, TrdgSesGrpDecoder, InstrumentDecoder, UndInstrmtGrpDecoder, LegOrdGrpDecoder, OrderQtyDataDecoder, TriggeringInstructionDecoder, CommissionDataDecoder, PegInstructionsDecoder, DiscretionInstructionsDecoder, StrategyParametersGrpDecoder, MessageDecoder
{
    ...
    public final IntHashSet GROUP_FIELDS = new IntHashSet(414);

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            GROUP_FIELDS.add(Constants.SWAP_POINTS);
    ...
    private final IntHashSet alreadyVisitedFields = new IntHashSet(414);
    ...
    public final IntHashSet messageFields = new IntHashSet(946);
 ...
 return CPProgram.NULL_VAL;
    ...
    private DecimalFloat swapPoints = DecimalFloat.newNaNValue();
    ...
    public DecimalFloat swapPoints()
    {
        if (!hasSwapPoints)
        {
            throw new IllegalArgumentException("No value for optional field: SwapPoints");
        }
 ...
 return MultiLegRptTypeReq.NULL_VAL;
    ...
    {
        // Decode MultilegOrderCancelReplace
     ...
     * {@inheritDoc}
     */
    public MultilegOrderCancelReplaceEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((MultilegOrderCancelReplaceEncoder)encoder);
    }

    public MultilegOrderCancelReplaceEncoder toEncoder(final MultilegOrderCancelReplaceEncoder encoder)
    {
        encoder.reset();
        if (hasOrderID())
        {
        ...
        final PreAllocMlegGrpEncoder preAllocMlegGrp = encoder.preAllocMlegGrp();        if (hasNoAllocsGroupCounter)
        {
            final int size = this.noAllocsGroupCounter;
            AllocsGroupDecoder allocsGroup = this.allocsGroup;
            AllocsGroupEncoder allocsGroupEncoder = preAllocMlegGrp.allocsGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (allocsGroup != null)
                {
        ...
        final LegOrdGrpEncoder legOrdGrp = encoder.legOrdGrp();        if (hasNoLegsGroupCounter)
        {
            final int size = this.noLegsGroupCounter;
            LegsGroupDecoder legsGroup = this.legsGroup;
            LegsGroupEncoder legsGroupEncoder = legOrdGrp.legsGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (legsGroup != null)
                {
