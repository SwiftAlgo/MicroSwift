...
public static class PosAmtGroupEncoder
{
    ...
    private static final int positionCurrencyHeaderLength = 5;
    private static final byte[] positionCurrencyHeader = new byte[] {49, 48, 53, 53, (byte) '='};
    ...
    private final MutableDirectBuffer positionCurrency = new UnsafeBuffer();

    private int positionCurrencyOffset = 0;

    private int positionCurrencyLength = 0;

    public PosAmtGroupEncoder positionCurrency(final DirectBuffer value, final int offset, final int length)
    {
        positionCurrency.wrap(value);
        positionCurrencyOffset = offset;
        positionCurrencyLength = length;
        return this;
    }

    public PosAmtGroupEncoder positionCurrency(final DirectBuffer value, final int length)
    {
        return positionCurrency(value, 0, length);
    }

    public PosAmtGroupEncoder positionCurrency(final DirectBuffer value)
    {
        return positionCurrency(value, 0, value.capacity());
    }

    public PosAmtGroupEncoder positionCurrency(final byte[] value, final int offset, final int length)
    {
        positionCurrency.wrap(value);
        positionCurrencyOffset = offset;
        positionCurrencyLength = length;
    ...
    public PosAmtGroupEncoder positionCurrencyAsCopy(final byte[] value, final int offset, final int length)
    {
        copyInto(positionCurrency, value, offset, length);
        positionCurrencyOffset = offset;
        positionCurrencyLength = length;
        return this;
    }

    public PosAmtGroupEncoder positionCurrency(final byte[] value, final int length)
    {
        return positionCurrency(value, 0, length);
    }

    public PosAmtGroupEncoder positionCurrency(final byte[] value)
    {
        return positionCurrency(value, 0, value.length);
    }

    public boolean hasPositionCurrency()
    {
        return positionCurrencyLength > 0;
    }

    public MutableDirectBuffer positionCurrency()
    {
    ...
    {
        return positionCurrency.getStringWithoutLengthAscii(positionCurrencyOffset, positionCurrencyLength);
    }

    public PosAmtGroupEncoder positionCurrency(final CharSequence value)
    {
        toBytes(value, positionCurrency);
        positionCurrencyOffset = 0;
        positionCurrencyLength = value.length();
        return this;
    }

    public PosAmtGroupEncoder positionCurrency(final AsciiSequenceView value)
    {
        final DirectBuffer buffer = value.buffer();
        if (buffer != null)
        {
            positionCurrency.wrap(buffer);
            positionCurrencyOffset = value.offset();
            positionCurrencyLength = value.length();
        }
        return this;
    }

    public PosAmtGroupEncoder positionCurrency(final char[] value)
    {
        return positionCurrency(value, 0, value.length);
    }

    public PosAmtGroupEncoder positionCurrency(final char[] value, final int length)
    {
        return positionCurrency(value, 0, length);
    }

    public PosAmtGroupEncoder positionCurrency(final char[] value, final int offset, final int length)
    {
        toBytes(value, positionCurrency, offset, length);
        positionCurrencyOffset = 0;
        positionCurrencyLength = length;
        return this;
    }
    ...
    public int encode(final MutableAsciiBuffer buffer, final int offset, final int remainingElements)
    {
        ...
        if (positionCurrencyLength > 0)
        {
            buffer.putBytes(position, positionCurrencyHeader, 0, positionCurrencyHeaderLength);
            position += positionCurrencyHeaderLength;
            buffer.putBytes(position, positionCurrency, positionCurrencyOffset, positionCurrencyLength);
            position += positionCurrencyLength;
            buffer.putSeparator(position);
            position++;
        }
        if (next != null)
    ...
    public StringBuilder appendTo(final StringBuilder builder, final int level)
    {
        ...
        if (hasPositionCurrency())
        {
            ...
            appendBuffer(builder, positionCurrency, positionCurrencyOffset, positionCurrencyLength);
            builder.append("\",\n");
        }
    ...
    public PosAmtGroupEncoder copyTo(final PosAmtGroupEncoder encoder)
    {
        ...
        if (hasPositionCurrency())
        {
            encoder.positionCurrencyAsCopy(positionCurrency.byteArray(), 0, positionCurrencyLength);
        }
        return encoder;
    }
