...
import com.swiftalgo.ms.codecs.fix50sp2.builder.SecurityStatusEncoder;
...
public class SecurityStatusDecoder extends CommonDecoderImpl implements ApplicationSequenceControlDecoder, InstrumentDecoder, InstrumentExtensionDecoder, UndInstrmtGrpDecoder, InstrmtLegGrpDecoder, MessageDecoder
{
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            GROUP_FIELDS.add(Constants.SECURITY_TRADING_EVENT);
    ...
    public boolean validate()
    {
        ...
        {
        if (CODEC_REJECT_UNKNOWN_ENUM_VALUE_ENABLED && !SecurityTradingEvent.isValid(securityTradingEvent()))
        {
            invalidTagId = 1174;
            rejectReason = 5;
            return false;
    ...
    public final IntHashSet messageFields = new IntHashSet(644);

    {
        ...
        messageFields.add(Constants.SECURITY_TRADING_EVENT);
    ...
    public void securityStatusReqID(final AsciiSequenceView view)
    {
        if (!hasSecurityStatusReqID)
        {
            throw new IllegalArgumentException("No value for optional field: SecurityStatusReqID");
        ...
        view.wrap(buffer, securityStatusReqIDOffset, securityStatusReqIDLength);
    }
 ...
 return SecurityTradingStatus.NULL_VAL;
    ...
    private int securityTradingEvent = MISSING_INT;
    ...
    public int securityTradingEvent()
    {
        if (!hasSecurityTradingEvent)
        {
            throw new IllegalArgumentException("No value for optional field: SecurityTradingEvent");
        }
    ...
    private final CharArrayWrapper securityTradingEventWrapper = new CharArrayWrapper();
    public SecurityTradingEvent securityTradingEventAsEnum()
    {
        if (!hasSecurityTradingEvent)
 return SecurityTradingEvent.NULL_VAL;
        return SecurityTradingEvent.decode(securityTradingEvent);
    }
 ...
 return Adjustment.NULL_VAL;
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.SECURITY_TRADING_EVENT:
                hasSecurityTradingEvent = true;
                securityTradingEvent = getInt(buffer, valueOffset, endOfField, 1174, CODEC_VALIDATION_ENABLED);
                break;
            ...
            case Constants.BUY_VOLUME:
                hasBuyVolume = true;
                buyVolume = getFloat(buffer, buyVolume, valueOffset, valueLength, 330, CODEC_VALIDATION_ENABLED);
                break;

            case Constants.SELL_VOLUME:
                hasSellVolume = true;
                sellVolume = getFloat(buffer, sellVolume, valueOffset, valueLength, 331, CODEC_VALIDATION_ENABLED);
                break;
