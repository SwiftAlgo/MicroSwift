...
public class RelatedSymGroupDecoder extends CommonDecoderImpl implements InstrumentDecoder, InstrumentExtensionDecoder, FinancingDetailsDecoder, SecurityTradingRulesDecoder, StrikeRulesDecoder, UndInstrmtGrpDecoder, StipulationsDecoder, SecLstUpdRelSymsLegGrpDecoder, SpreadOrBenchmarkCurveDataDecoder, YieldDataDecoder
{
    ...
    public final IntHashSet GROUP_FIELDS = new IntHashSet(258);
 ...
 return YieldType.NULL_VAL;
    ...
    public RelatedSymGroupEncoder toEncoder(final RelatedSymGroupEncoder encoder)
    {
        ...
        final SecLstUpdRelSymsLegGrpEncoder secLstUpdRelSymsLegGrp = encoder.secLstUpdRelSymsLegGrp();        if (hasNoLegsGroupCounter)
        {
            final int size = this.noLegsGroupCounter;
            LegsGroupDecoder legsGroup = this.legsGroup;
            LegsGroupEncoder legsGroupEncoder = secLstUpdRelSymsLegGrp.legsGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (legsGroup != null)
                {
    ...
    {
        private final SecLstUpdRelSymGrpDecoder parent;
        private int remainder;
        private RelatedSymGroupDecoder current;

        public RelatedSymGroupIterator(final SecLstUpdRelSymGrpDecoder parent)
        {
            this.parent = parent;
        }
