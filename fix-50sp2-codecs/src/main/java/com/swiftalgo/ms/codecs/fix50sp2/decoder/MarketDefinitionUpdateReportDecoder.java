...
import com.swiftalgo.ms.codecs.fix50sp2.builder.MarketDefinitionUpdateReportEncoder;
...
public class MarketDefinitionUpdateReportDecoder extends CommonDecoderImpl implements ApplicationSequenceControlDecoder, BaseTradingRulesDecoder, OrdTypeRulesDecoder, TimeInForceRulesDecoder, ExecInstRulesDecoder, MessageDecoder
{
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            GROUP_FIELDS.add(Constants.MARKET_UPDATE_ACTION);
    ...
    private final IntHashSet alreadyVisitedFields = new IntHashSet(64);
    ...
    public boolean validate()
    {
        ...
        {
        if (CODEC_REJECT_UNKNOWN_ENUM_VALUE_ENABLED && !MarketUpdateAction.isValid(marketUpdateAction()))
        {
            invalidTagId = 1395;
            rejectReason = 5;
            return false;
    ...
    public static final long MESSAGE_TYPE = 22082L;

    public static final String MESSAGE_TYPE_AS_STRING = "BV";
...

    {
        ...
        messageFields.add(Constants.MARKET_UPDATE_ACTION);
    ...
    private char marketUpdateAction = MISSING_CHAR;
    ...
    public char marketUpdateAction()
    {
        if (!hasMarketUpdateAction)
        {
            throw new IllegalArgumentException("No value for optional field: MarketUpdateAction");
        }
    ...
    private final CharArrayWrapper marketUpdateActionWrapper = new CharArrayWrapper();
    public MarketUpdateAction marketUpdateActionAsEnum()
    {
        if (!hasMarketUpdateAction)
 return MarketUpdateAction.NULL_VAL;
        return MarketUpdateAction.decode(marketUpdateAction);
    }
 ...
 return PriceType.NULL_VAL;
    ...
    {
        // Decode MarketDefinitionUpdateReport
        ...
        while (position < end)
        {
            ...
            case Constants.MARKET_UPDATE_ACTION:
                hasMarketUpdateAction = true;
                marketUpdateAction = buffer.getChar(valueOffset);
                break;
     ...
     * {@inheritDoc}
     */
    public MarketDefinitionUpdateReportEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((MarketDefinitionUpdateReportEncoder)encoder);
    }

    public MarketDefinitionUpdateReportEncoder toEncoder(final MarketDefinitionUpdateReportEncoder encoder)
    {
        encoder.reset();
