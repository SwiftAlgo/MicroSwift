...
import com.swiftalgo.ms.codecs.fix50sp2.builder.TradeCaptureReportEncoder;
...
import com.swiftalgo.ms.codecs.fix50sp2.builder.TrdCapRptSideGrpEncoder;
import static com.swiftalgo.ms.codecs.fix50sp2.builder.TrdCapRptSideGrpEncoder.SidesGroupEncoder;
...
public class TradeCaptureReportDecoder extends CommonDecoderImpl implements ApplicationSequenceControlDecoder, RootPartiesDecoder, InstrumentDecoder, FinancingDetailsDecoder, YieldDataDecoder, UndInstrmtGrpDecoder, SpreadOrBenchmarkCurveDataDecoder, PositionAmountDataDecoder, TrdInstrmtLegGrpDecoder, TrdRegTimestampsDecoder, TrdCapRptSideGrpDecoder, TrdRepIndicatorsGrpDecoder, MessageDecoder
{
    ...
    public final IntHashSet GROUP_FIELDS = new IntHashSet(384);

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            GROUP_FIELDS.add(Constants.UNDERLYING_SETTLEMENT_DATE);
            ...
            GROUP_FIELDS.add(Constants.CURRENCY_RATIO);
            ...
            GROUP_FIELDS.add(Constants.T_Z_TRANSACT_TIME);
            GROUP_FIELDS.add(Constants.REPORTED_PX_DIFF);
            GROUP_FIELDS.add(Constants.GROSS_TRADE_AMT);
            GROUP_FIELDS.add(Constants.REJECT_TEXT);
            GROUP_FIELDS.add(Constants.FEE_MULTIPLIER);
        }
    }

    private final IntHashSet alreadyVisitedFields = new IntHashSet(384);
    ...
    public final IntHashSet messageFields = new IntHashSet(1222);

    {
        ...
        messageFields.add(Constants.EXCHANGE_SPECIAL_INSTRUCTIONS);
        messageFields.add(Constants.ORDER_CATEGORY);
        messageFields.add(Constants.SIDE_LIQUIDITY_IND);
        ...
        messageFields.add(Constants.T_Z_TRANSACT_TIME);
        messageFields.add(Constants.REPORTED_PX_DIFF);
        messageFields.add(Constants.GROSS_TRADE_AMT);
        messageFields.add(Constants.REJECT_TEXT);
 ...
 return MatchType.NULL_VAL;
    ...
    private DecimalFloat currencyRatio = DecimalFloat.newNaNValue();
    ...
    public DecimalFloat currencyRatio()
    {
        if (!hasCurrencyRatio)
        {
            throw new IllegalArgumentException("No value for optional field: CurrencyRatio");
        }
 ...
 return ShortSaleReason.NULL_VAL;
    ...
    private byte[] tZTransactTime = new byte[31];

    private boolean hasTZTransactTime;

    public byte[] tZTransactTime()
    {
        if (!hasTZTransactTime)
        {
            throw new IllegalArgumentException("No value for optional field: TZTransactTime");
        }
    ...
    {
        return hasTZTransactTime;
    }


    private int tZTransactTimeOffset;

    private int tZTransactTimeLength;

    public int tZTransactTimeLength()
    {
        if (!hasTZTransactTime)
        {
            throw new IllegalArgumentException("No value for optional field: TZTransactTime");
        ...
        return tZTransactTimeLength;
    }

    public String tZTransactTimeAsString()
    {
        return hasTZTransactTime ? new String(tZTransactTime, 0, tZTransactTimeLength) : null;
    }

    public void tZTransactTime(final AsciiSequenceView view)
    {
        if (!hasTZTransactTime)
        {
            throw new IllegalArgumentException("No value for optional field: TZTransactTime");
        ...
        view.wrap(buffer, tZTransactTimeOffset, tZTransactTimeLength);
    }
    ...
    public boolean reportedPxDiff()
    {
        ...
        {
            throw new IllegalArgumentException("No value for optional field: ReportedPxDiff");
        }
    ...
    private char[] rejectText = new char[1];

    private boolean hasRejectText;

    public char[] rejectText()
    {
        if (!hasRejectText)
        {
            throw new IllegalArgumentException("No value for optional field: RejectText");
        }
    ...
    {
        return hasRejectText;
    }


    private int rejectTextOffset;

    private int rejectTextLength;
    ...
    public int rejectTextLength()
    {
        ...
        return rejectTextLength;
    }

    public String rejectTextAsString()
    {
        return hasRejectText ? new String(rejectText, 0, rejectTextLength) : null;
    }

    public void rejectText(final AsciiSequenceView view)
    {
        if (!hasRejectText)
        {
            throw new IllegalArgumentException("No value for optional field: RejectText");
        ...
        view.wrap(buffer, rejectTextOffset, rejectTextLength);
    }
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.CURRENCY_RATIO:
                hasCurrencyRatio = true;
                currencyRatio = getFloat(buffer, currencyRatio, valueOffset, valueLength, 1382, CODEC_VALIDATION_ENABLED);
                break;
            ...
            case Constants.T_Z_TRANSACT_TIME:
                hasTZTransactTime = true;
                tZTransactTime = buffer.getBytes(tZTransactTime, valueOffset, valueLength);
                tZTransactTimeOffset = valueOffset;
                tZTransactTimeLength = valueLength;
                break;

            case Constants.REPORTED_PX_DIFF:
                hasReportedPxDiff = true;
                reportedPxDiff = buffer.getBoolean(valueOffset);
                break;
            ...
            case Constants.REJECT_TEXT:
                hasRejectText = true;
                rejectText = buffer.getChars(rejectText, valueOffset, valueLength);
                rejectTextOffset = valueOffset;
                rejectTextLength = valueLength;
                break;
    ...
    if (hasNoTrdRepIndicatorsGroupCounter)
    {
        ...
        if (hasTZTransactTime())
        {
            ...
            appendData(builder, tZTransactTime, tZTransactTimeLength);
            builder.append("\",\n");
        }
        ...
        if (hasRejectText())
        {
            ...
            builder.append(this.rejectText(), 0, rejectTextLength());
            builder.append("\",\n");
        }
    ...
    public TradeCaptureReportEncoder toEncoder(final TradeCaptureReportEncoder encoder)
    {
        ...
        final TrdCapRptSideGrpEncoder trdCapRptSideGrp = encoder.trdCapRptSideGrp();        if (hasNoSidesGroupCounter)
        {
            final int size = this.noSidesGroupCounter;
            SidesGroupDecoder sidesGroup = this.sidesGroup;
            SidesGroupEncoder sidesGroupEncoder = trdCapRptSideGrp.sidesGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (sidesGroup != null)
                {
        ...
        if (hasTZTransactTime())
        {
            encoder.tZTransactTimeAsCopy(this.tZTransactTime(), 0, tZTransactTimeLength());
        }
        ...
        if (hasRejectText())
        {
            encoder.rejectText(this.rejectText(), 0, rejectTextLength());
        }
