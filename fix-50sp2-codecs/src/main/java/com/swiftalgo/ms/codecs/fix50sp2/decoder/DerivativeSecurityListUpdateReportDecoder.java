...
import com.swiftalgo.ms.codecs.fix50sp2.builder.DerivativeSecurityListUpdateReportEncoder;
...
import com.swiftalgo.ms.codecs.fix50sp2.builder.DerivativeSecurityDefinitionEncoder;
...
import com.swiftalgo.ms.codecs.fix50sp2.builder.RelSymDerivSecUpdGrpEncoder;
import static com.swiftalgo.ms.codecs.fix50sp2.builder.RelSymDerivSecUpdGrpEncoder.RelatedSymGroupEncoder;
...
import com.swiftalgo.ms.codecs.fix50sp2.builder.SecondaryPriceLimitsEncoder;
...
public class DerivativeSecurityListUpdateReportDecoder extends CommonDecoderImpl implements ApplicationSequenceControlDecoder, UnderlyingInstrumentDecoder, DerivativeSecurityDefinitionDecoder, RelSymDerivSecUpdGrpDecoder, MessageDecoder
{
    ...
    private final IntHashSet alreadyVisitedFields = new IntHashSet(278);
    ...
    public boolean validate()
    {
        ...
        if (hasNoDerivativeInstrAttribGroupCounter)
        {
            {
                int count = 0;
                final DerivativeInstrAttribGroupIterator iterator = derivativeInstrAttribGroupIterator.iterator();
                for (final DerivativeInstrAttribGroupDecoder group : iterator)
                {
                    count++;                    if (!group.validate())
                    {
                        invalidTagId = group.invalidTagId();
        ...
        if (hasNoMarketSegmentsGroupCounter)
        {
            {
                int count = 0;
                final MarketSegmentsGroupIterator iterator = marketSegmentsGroupIterator.iterator();
                for (final MarketSegmentsGroupDecoder group : iterator)
                {
                    count++;                    if (!group.validate())
                    {
                        invalidTagId = group.invalidTagId();
    ...
    public static final long MESSAGE_TYPE = 21058L;

    public static final String MESSAGE_TYPE_AS_STRING = "BR";

    public static final char[] MESSAGE_TYPE_CHARS = MESSAGE_TYPE_AS_STRING.toCharArray();

    public static final byte[] MESSAGE_TYPE_BYTES = MESSAGE_TYPE_AS_STRING.getBytes(US_ASCII);

    public final IntHashSet messageFields = new IntHashSet(896);

    {
        ...
        messageFields.add(Constants.SECONDARY_LOW_LIMIT_PRICE);
 ...
 return UnderlyingFXRateCalc.NULL_VAL;
    ...
    private DerivativeInstrAttribGroupDecoder derivativeInstrAttribGroup = null;
    public DerivativeInstrAttribGroupDecoder derivativeInstrAttribGroup()
    {
        return derivativeInstrAttribGroup;
    }

    private int noDerivativeInstrAttribGroupCounter = MISSING_INT;
    ...
    public int noDerivativeInstrAttribGroupCounter()
    {
        if (!hasNoDerivativeInstrAttribGroupCounter)
        {
            throw new IllegalArgumentException("No value for optional field: NoDerivativeInstrAttribGroupCounter");
        }
    ...
    private DerivativeInstrAttribGroupIterator derivativeInstrAttribGroupIterator = new DerivativeInstrAttribGroupIterator(this);
    public DerivativeInstrAttribGroupIterator derivativeInstrAttribGroupIterator()
    {
        return derivativeInstrAttribGroupIterator.iterator();
    }




    private MarketSegmentsGroupDecoder marketSegmentsGroup = null;
    public MarketSegmentsGroupDecoder marketSegmentsGroup()
    {
        return marketSegmentsGroup;
    }

    private int noMarketSegmentsGroupCounter = MISSING_INT;
    ...
    public int noMarketSegmentsGroupCounter()
    {
        if (!hasNoMarketSegmentsGroupCounter)
        {
            throw new IllegalArgumentException("No value for optional field: NoMarketSegmentsGroupCounter");
        }
    ...
    private MarketSegmentsGroupIterator marketSegmentsGroupIterator = new MarketSegmentsGroupIterator(this);
    public MarketSegmentsGroupIterator marketSegmentsGroupIterator()
    {
        return marketSegmentsGroupIterator.iterator();
    }
    ...
    {
        // Decode DerivativeSecurityListUpdateReport
        ...
        while (position < end)
        {
            ...
            case Constants.NO_DERIVATIVE_INSTR_ATTRIB_GROUP_COUNTER:
                ...
                {
                    derivativeInstrAttribGroup = new DerivativeInstrAttribGroupDecoder(trailer, messageFields);
                }
                DerivativeInstrAttribGroupDecoder derivativeInstrAttribGroupCurrent = derivativeInstrAttribGroup;
                position = endOfField + 1;
                final int noDerivativeInstrAttribGroupCounter = this.noDerivativeInstrAttribGroupCounter;
                for (int i = 0; i < noDerivativeInstrAttribGroupCounter && position < end; i++)
                {
                    if (derivativeInstrAttribGroupCurrent != null)
                    {
                        position += derivativeInstrAttribGroupCurrent.decode(buffer, position, end - position);
                        derivativeInstrAttribGroupCurrent = derivativeInstrAttribGroupCurrent.next();
                    }
                }
                if (CODEC_VALIDATION_ENABLED)
                {
                    ...
                    {
                        final int checkTag = buffer.getInt(position, checkEqualsPosition);
                        if (derivativeInstrAttribGroup.ALL_GROUP_FIELDS.contains(checkTag))
                        {
                            invalidTagId = tag;
                            rejectReason = 16;
                            return position;
            ...
            case Constants.NO_MARKET_SEGMENTS_GROUP_COUNTER:
                ...
                {
                    marketSegmentsGroup = new MarketSegmentsGroupDecoder(trailer, messageFields);
                }
                MarketSegmentsGroupDecoder marketSegmentsGroupCurrent = marketSegmentsGroup;
                position = endOfField + 1;
                final int noMarketSegmentsGroupCounter = this.noMarketSegmentsGroupCounter;
                for (int i = 0; i < noMarketSegmentsGroupCounter && position < end; i++)
                {
                    if (marketSegmentsGroupCurrent != null)
                    {
                        position += marketSegmentsGroupCurrent.decode(buffer, position, end - position);
                        marketSegmentsGroupCurrent = marketSegmentsGroupCurrent.next();
                    }
                }
                if (CODEC_VALIDATION_ENABLED)
                {
                    ...
                    {
                        final int checkTag = buffer.getInt(position, checkEqualsPosition);
                        if (marketSegmentsGroup.ALL_GROUP_FIELDS.contains(checkTag))
                        {
                            invalidTagId = tag;
                            rejectReason = 16;
                            return position;
    ...
    {
        for (final DerivativeInstrAttribGroupDecoder derivativeInstrAttribGroupDecoder : derivativeInstrAttribGroupIterator.iterator())
        {
            derivativeInstrAttribGroupDecoder.reset();
            if (derivativeInstrAttribGroupDecoder.next() == null)
            {
                break;
            }
        }
        noDerivativeInstrAttribGroupCounter = MISSING_INT;
        hasNoDerivativeInstrAttribGroupCounter = false;
    }

    public void resetMarketSegmentsGroup()
    {
        for (final MarketSegmentsGroupDecoder marketSegmentsGroupDecoder : marketSegmentsGroupIterator.iterator())
        {
            marketSegmentsGroupDecoder.reset();
            if (marketSegmentsGroupDecoder.next() == null)
            {
                break;
            }
        }
        noMarketSegmentsGroupCounter = MISSING_INT;
     ...
     * {@inheritDoc}
     */
    public DerivativeSecurityListUpdateReportEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((DerivativeSecurityListUpdateReportEncoder)encoder);
    }

    public DerivativeSecurityListUpdateReportEncoder toEncoder(final DerivativeSecurityListUpdateReportEncoder encoder)
    {
        encoder.reset();
        ...
        final DerivativeSecurityDefinitionEncoder derivativeSecurityDefinition = encoder.derivativeSecurityDefinition();
        final DerivativeInstrumentEncoder derivativeInstrument = derivativeSecurityDefinition.derivativeInstrument();        if (hasDerivativeSymbol())
        {
        ...
        final DerivativeInstrumentAttributeEncoder derivativeInstrumentAttribute = derivativeSecurityDefinition.derivativeInstrumentAttribute();        if (hasNoDerivativeInstrAttribGroupCounter)
        {
            final int size = this.noDerivativeInstrAttribGroupCounter;
            DerivativeInstrAttribGroupDecoder derivativeInstrAttribGroup = this.derivativeInstrAttribGroup;
            DerivativeInstrAttribGroupEncoder derivativeInstrAttribGroupEncoder = derivativeInstrumentAttribute.derivativeInstrAttribGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (derivativeInstrAttribGroup != null)
                {
                    derivativeInstrAttribGroup.toEncoder(derivativeInstrAttribGroupEncoder);
                    derivativeInstrAttribGroup = derivativeInstrAttribGroup.next();
        ...
        final MarketSegmentGrpEncoder marketSegmentGrp = derivativeSecurityDefinition.marketSegmentGrp();        if (hasNoMarketSegmentsGroupCounter)
        {
            final int size = this.noMarketSegmentsGroupCounter;
            MarketSegmentsGroupDecoder marketSegmentsGroup = this.marketSegmentsGroup;
            MarketSegmentsGroupEncoder marketSegmentsGroupEncoder = marketSegmentGrp.marketSegmentsGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (marketSegmentsGroup != null)
                {
                    marketSegmentsGroup.toEncoder(marketSegmentsGroupEncoder);
                    marketSegmentsGroup = marketSegmentsGroup.next();
        ...
        final RelSymDerivSecUpdGrpEncoder relSymDerivSecUpdGrp = encoder.relSymDerivSecUpdGrp();        if (hasNoRelatedSymGroupCounter)
        {
            final int size = this.noRelatedSymGroupCounter;
            RelatedSymGroupDecoder relatedSymGroup = this.relatedSymGroup;
            RelatedSymGroupEncoder relatedSymGroupEncoder = relSymDerivSecUpdGrp.relatedSymGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (relatedSymGroup != null)
                {
