...
import com.swiftalgo.ms.codecs.fix50sp2.builder.MarketDataSnapshotFullRefreshEncoder;
...
public class MarketDataSnapshotFullRefreshDecoder extends CommonDecoderImpl implements ApplicationSequenceControlDecoder, InstrumentDecoder, UndInstrmtGrpDecoder, InstrmtLegGrpDecoder, MDFullGrpDecoder, RoutingGrpDecoder, MessageDecoder
{
    ...
    public final IntHashSet GROUP_FIELDS = new IntHashSet(208);

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            GROUP_FIELDS.add(Constants.M_D_REPORT_ID);
            ...
            GROUP_FIELDS.add(Constants.REFRESH_INDICATOR);
    ...
    private final IntHashSet alreadyVisitedFields = new IntHashSet(208);
    ...
    public final IntHashSet messageFields = new IntHashSet(790);

    {
        ...
        messageFields.add(Constants.M_D_REPORT_ID);
        ...
        messageFields.add(Constants.REFRESH_INDICATOR);
    ...
    private int mDReportID = MISSING_INT;
    ...
    public int mDReportID()
    {
        if (!hasMDReportID)
        {
            throw new IllegalArgumentException("No value for optional field: MDReportID");
        }
 ...
 return MDBookType.NULL_VAL;
    ...
    public boolean refreshIndicator()
    {
        ...
        {
            throw new IllegalArgumentException("No value for optional field: RefreshIndicator");
        }
 ...
 return ApplQueueResolution.NULL_VAL;
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.M_D_REPORT_ID:
                hasMDReportID = true;
                mDReportID = getInt(buffer, valueOffset, endOfField, 963, CODEC_VALIDATION_ENABLED);
                break;
            ...
            case Constants.REFRESH_INDICATOR:
                hasRefreshIndicator = true;
                refreshIndicator = buffer.getBoolean(valueOffset);
                break;
    ...
    public MarketDataSnapshotFullRefreshEncoder toEncoder(final MarketDataSnapshotFullRefreshEncoder encoder)
    {
        ...
        final MDFullGrpEncoder mDFullGrp = encoder.mDFullGrp();        if (hasNoMDEntriesGroupCounter)
        {
            final int size = this.noMDEntriesGroupCounter;
            MDEntriesGroupDecoder mDEntriesGroup = this.mDEntriesGroup;
            MDEntriesGroupEncoder mDEntriesGroupEncoder = mDFullGrp.mDEntriesGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (mDEntriesGroup != null)
                {
