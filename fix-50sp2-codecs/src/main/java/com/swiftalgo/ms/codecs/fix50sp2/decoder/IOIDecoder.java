...
import com.swiftalgo.ms.codecs.fix50sp2.builder.IOIEncoder;
...
import com.swiftalgo.ms.codecs.fix50sp2.builder.InstrmtLegIOIGrpEncoder;
import static com.swiftalgo.ms.codecs.fix50sp2.builder.InstrmtLegIOIGrpEncoder.LegsGroupEncoder;
...
public class IOIDecoder extends CommonDecoderImpl implements ApplicationSequenceControlDecoder, InstrumentDecoder, PartiesDecoder, FinancingDetailsDecoder, UndInstrmtGrpDecoder, OrderQtyDataDecoder, StipulationsDecoder, InstrmtLegIOIGrpDecoder, IOIQualGrpDecoder, RoutingGrpDecoder, SpreadOrBenchmarkCurveDataDecoder, YieldDataDecoder, MessageDecoder
{
    ...
    public final IntHashSet GROUP_FIELDS = new IntHashSet(266);
    ...
    private final IntHashSet alreadyVisitedFields = new IntHashSet(266);
    ...
    public boolean validate()
    {
        ...
        iOIQtyWrapper.wrap(this.iOIQty(), iOIQtyLength);
        if (CODEC_REJECT_UNKNOWN_ENUM_VALUE_ENABLED && !IOIQty.isValid(iOIQtyWrapper))
        {
            invalidTagId = 27;
            rejectReason = 5;
            return false;
    ...
    public final IntHashSet messageFields = new IntHashSet(708);

    {
        ...
        messageFields.add(Constants.NO_I_O_I_QUALIFIERS);
    ...
    public void iOIRefID(final AsciiSequenceView view)
    {
        if (!hasIOIRefID)
        {
            throw new IllegalArgumentException("No value for optional field: IOIRefID");
        ...
        view.wrap(buffer, iOIRefIDOffset, iOIRefIDLength);
    }
 ...
 return RoundingDirection.NULL_VAL;
    ...
    public void iOIQty(final AsciiSequenceView view)
    {
        view.wrap(buffer, iOIQtyOffset, iOIQtyLength);
    }


    private final CharArrayWrapper iOIQtyWrapper = new CharArrayWrapper();
    public IOIQty iOIQtyAsEnum()
    {
        iOIQtyWrapper.wrap(this.iOIQty(), iOIQtyLength);
        return IOIQty.decode(iOIQtyWrapper);
    }
 ...
 return YieldType.NULL_VAL;
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.NO_I_O_I_QUALIFIERS:
    ...
    if (hasNoIOIQualifiersGroupCounter)
    {
        ...
        {
            indent(builder, level);
            iOIQualifiersGroup.appendTo(builder, level + 1);            if (iOIQualifiersGroup.next() != null)
            {
                builder.append(',');
            }
            builder.append('\n');
     ...
     * {@inheritDoc}
     */
    public IOIEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((IOIEncoder)encoder);
    }

    public IOIEncoder toEncoder(final IOIEncoder encoder)
    {
        encoder.reset();
        ...
        final InstrmtLegIOIGrpEncoder instrmtLegIOIGrp = encoder.instrmtLegIOIGrp();        if (hasNoLegsGroupCounter)
        {
            final int size = this.noLegsGroupCounter;
            LegsGroupDecoder legsGroup = this.legsGroup;
            LegsGroupEncoder legsGroupEncoder = instrmtLegIOIGrp.legsGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (legsGroup != null)
                {
        ...
        final IOIQualGrpEncoder iOIQualGrp = encoder.iOIQualGrp();        if (hasNoIOIQualifiersGroupCounter)
        {
            final int size = this.noIOIQualifiersGroupCounter;
            IOIQualifiersGroupDecoder iOIQualifiersGroup = this.iOIQualifiersGroup;
            IOIQualifiersGroupEncoder iOIQualifiersGroupEncoder = iOIQualGrp.iOIQualifiersGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (iOIQualifiersGroup != null)
                {
