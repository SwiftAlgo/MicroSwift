...
public interface MDIncGrpDecoder 
{



public class MDEntriesGroupDecoder extends CommonDecoderImpl implements InstrumentDecoder, UndInstrmtGrpDecoder, InstrmtLegGrpDecoder, YieldDataDecoder, SpreadOrBenchmarkCurveDataDecoder, RateSourceDecoder, SecSizesGrpDecoder, StatsIndGrpDecoder, PartiesDecoder
{
    ...
    public final IntHashSet GROUP_FIELDS = new IntHashSet(336);

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            GROUP_FIELDS.add(Constants.M_D_SUB_BOOK_TYPE);
            ...
            GROUP_FIELDS.add(Constants.M_D_STREAM_ID);
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            ALL_GROUP_FIELDS.add(Constants.M_D_SUB_BOOK_TYPE);
            ...
            ALL_GROUP_FIELDS.add(Constants.M_D_STREAM_ID);
    ...
    public boolean validate()
    {
        ...
        if (hasNoStatsIndicatorsGroupCounter)
        {
            {
                int count = 0;
                final StatsIndicatorsGroupIterator iterator = statsIndicatorsGroupIterator.iterator();
                for (final StatsIndicatorsGroupDecoder group : iterator)
                {
                    count++;                    if (!group.validate())
                    {
                        invalidTagId = group.invalidTagId();
    ...
    private IntHashSet seenFields = new IntHashSet(158);
 ...
 return DeleteReason.NULL_VAL;
    ...
    private int mDSubBookType = MISSING_INT;
    ...
    public int mDSubBookType()
    {
        if (!hasMDSubBookType)
        {
            throw new IllegalArgumentException("No value for optional field: MDSubBookType");
        }
 ...
 return MDEntryType.NULL_VAL;
    ...
    public void mDEntryRefID(final AsciiSequenceView view)
    {
        if (!hasMDEntryRefID)
        {
            throw new IllegalArgumentException("No value for optional field: MDEntryRefID");
        ...
        view.wrap(buffer, mDEntryRefIDOffset, mDEntryRefIDLength);
    }
    ...
    public char[] mDStreamID()
    {
        if (!hasMDStreamID)
        {
            throw new IllegalArgumentException("No value for optional field: MDStreamID");
        }
    ...
    {
        return hasMDStreamID;
    }


    private int mDStreamIDOffset;

    private int mDStreamIDLength;

    public int mDStreamIDLength()
    {
        if (!hasMDStreamID)
        {
            throw new IllegalArgumentException("No value for optional field: MDStreamID");
        ...
        return mDStreamIDLength;
    }

    public String mDStreamIDAsString()
    {
        return hasMDStreamID ? new String(mDStreamID, 0, mDStreamIDLength) : null;
    }

    public void mDStreamID(final AsciiSequenceView view)
    {
        if (!hasMDStreamID)
        {
            throw new IllegalArgumentException("No value for optional field: MDStreamID");
        ...
        view.wrap(buffer, mDStreamIDOffset, mDStreamIDLength);
    }
 ...
 return DealingCapacity.NULL_VAL;
    ...
    private StatsIndicatorsGroupDecoder statsIndicatorsGroup = null;
    public StatsIndicatorsGroupDecoder statsIndicatorsGroup()
    {
        return statsIndicatorsGroup;
    }

    private int noStatsIndicatorsGroupCounter = MISSING_INT;
    ...
    public int noStatsIndicatorsGroupCounter()
    {
        if (!hasNoStatsIndicatorsGroupCounter)
        {
            throw new IllegalArgumentException("No value for optional field: NoStatsIndicatorsGroupCounter");
        }
    ...
    private StatsIndicatorsGroupIterator statsIndicatorsGroupIterator = new StatsIndicatorsGroupIterator(this);
    public StatsIndicatorsGroupIterator statsIndicatorsGroupIterator()
    {
        return statsIndicatorsGroupIterator.iterator();
    }
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.M_D_SUB_BOOK_TYPE:
                hasMDSubBookType = true;
                mDSubBookType = getInt(buffer, valueOffset, endOfField, 1173, CODEC_VALIDATION_ENABLED);
                break;
            ...
            case Constants.M_D_STREAM_ID:
                hasMDStreamID = true;
                mDStreamID = buffer.getChars(mDStreamID, valueOffset, valueLength);
                mDStreamIDOffset = valueOffset;
                mDStreamIDLength = valueLength;
                break;
            ...
            case Constants.NO_STATS_INDICATORS_GROUP_COUNTER:
                ...
                {
                    statsIndicatorsGroup = new StatsIndicatorsGroupDecoder(trailer, messageFields);
                }
                StatsIndicatorsGroupDecoder statsIndicatorsGroupCurrent = statsIndicatorsGroup;
                position = endOfField + 1;
                final int noStatsIndicatorsGroupCounter = this.noStatsIndicatorsGroupCounter;
                for (int i = 0; i < noStatsIndicatorsGroupCounter && position < end; i++)
                {
                    if (statsIndicatorsGroupCurrent != null)
                    {
                        position += statsIndicatorsGroupCurrent.decode(buffer, position, end - position);
                        statsIndicatorsGroupCurrent = statsIndicatorsGroupCurrent.next();
                    }
                }
                if (CODEC_VALIDATION_ENABLED)
                {
                    ...
                    {
                        final int checkTag = buffer.getInt(position, checkEqualsPosition);
                        if (statsIndicatorsGroup.ALL_GROUP_FIELDS.contains(checkTag))
                        {
                            invalidTagId = tag;
                            rejectReason = 16;
                            return position;
    ...
    {
        for (final StatsIndicatorsGroupDecoder statsIndicatorsGroupDecoder : statsIndicatorsGroupIterator.iterator())
        {
            statsIndicatorsGroupDecoder.reset();
            if (statsIndicatorsGroupDecoder.next() == null)
            {
                break;
            }
        }
        noStatsIndicatorsGroupCounter = MISSING_INT;
    ...
    public StringBuilder appendTo(final StringBuilder builder, final int level)
    {
        ...
        if (hasMDStreamID())
        {
            ...
            builder.append(this.mDStreamID(), 0, mDStreamIDLength());
            builder.append("\",\n");
        }
    ...
    public MDIncGrpEncoder.MDEntriesGroupEncoder toEncoder(final MDIncGrpEncoder.MDEntriesGroupEncoder encoder)
    {
        ...
        if (hasMDStreamID())
        {
            encoder.mDStreamID(this.mDStreamID(), 0, mDStreamIDLength());
        }
        ...
        final StatsIndGrpEncoder statsIndGrp = encoder.statsIndGrp();        if (hasNoStatsIndicatorsGroupCounter)
        {
            final int size = this.noStatsIndicatorsGroupCounter;
            StatsIndicatorsGroupDecoder statsIndicatorsGroup = this.statsIndicatorsGroup;
            StatsIndicatorsGroupEncoder statsIndicatorsGroupEncoder = statsIndGrp.statsIndicatorsGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (statsIndicatorsGroup != null)
                {
                    statsIndicatorsGroup.toEncoder(statsIndicatorsGroupEncoder);
                    statsIndicatorsGroup = statsIndicatorsGroup.next();
    ...
    {
        private final MDIncGrpDecoder parent;
        private int remainder;
        private MDEntriesGroupDecoder current;

        public MDEntriesGroupIterator(final MDIncGrpDecoder parent)
        {
            this.parent = parent;
        }
