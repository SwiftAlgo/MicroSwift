...
public class QuotEntryAckGrpEncoder
{
...
public static class QuoteEntriesGroupEncoder
{
    ...
    private static final int quoteEntryStatusHeaderLength = 5;
    private static final byte[] quoteEntryStatusHeader = new byte[] {49, 49, 54, 55, (byte) '='};
    ...
    public QuoteEntriesGroupEncoder quoteEntryStatus(int value)
    {
        quoteEntryStatus = value;
    ...
    public QuoteEntriesGroupEncoder quoteEntryStatus(QuoteEntryStatus value)
    {
        if (CODEC_VALIDATION_ENABLED)
        {
            if (value == QuoteEntryStatus.ARTIO_UNKNOWN)
            {
                throw new EncodingException("Invalid Value Field: quoteEntryStatus Value: " + value );
            }
            if (value == QuoteEntryStatus.NULL_VAL)
            {
                return this;
            }
        }
        return quoteEntryStatus(value.representation());
    }
    ...
    public int encode(final MutableAsciiBuffer buffer, final int offset, final int remainingElements)
    {
        ...
        {
            buffer.putBytes(position, quoteEntryStatusHeader, 0, quoteEntryStatusHeaderLength);
            position += quoteEntryStatusHeaderLength;
            position += buffer.putIntAscii(position, quoteEntryStatus);
            buffer.putSeparator(position);
            position++;
        }
    ...
    public QuotEntryAckGrpEncoder noQuoteEntriesGroupCounter(int value)
    {
    ...
    {
        builder.append("{\n");        indent(builder, level);
        builder.append("\"MessageName\": \"QuotEntryAckGrp\",\n");
        if (hasNoQuoteEntriesGroupCounter)
        {
            indent(builder, level);
     ...
     * {@inheritDoc}
     */
    public QuotEntryAckGrpEncoder copyTo(final Encoder encoder)
    {
        return copyTo((QuotEntryAckGrpEncoder)encoder);
    }

    public QuotEntryAckGrpEncoder copyTo(final QuotEntryAckGrpEncoder encoder)
    {
        encoder.reset();
        if (hasNoQuoteEntriesGroupCounter)
        {
