...
public interface ApplIDRequestAckGrpDecoder 
{
...
public class ApplIDsGroupDecoder extends CommonDecoderImpl implements NestedPartiesDecoder
{
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            GROUP_FIELDS.add(Constants.REF_APPL_LAST_SEQ_NUM);
            GROUP_FIELDS.add(Constants.APPL_RESPONSE_ERROR);
        }
    }
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            ALL_GROUP_FIELDS.add(Constants.APPL_RESPONSE_ERROR);
        }
    }
    ...
    public boolean validate()
    {
        ...
        {
        if (CODEC_REJECT_UNKNOWN_ENUM_VALUE_ENABLED && !ApplResponseError.isValid(applResponseError()))
        {
            invalidTagId = 1354;
            rejectReason = 5;
            return false;
    ...
    private int applResponseError = MISSING_INT;
    ...
    public int applResponseError()
    {
        if (!hasApplResponseError)
        {
            throw new IllegalArgumentException("No value for optional field: ApplResponseError");
        }
    ...
    private final CharArrayWrapper applResponseErrorWrapper = new CharArrayWrapper();
    public ApplResponseError applResponseErrorAsEnum()
    {
        if (!hasApplResponseError)
 return ApplResponseError.NULL_VAL;
        return ApplResponseError.decode(applResponseError);
    }
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.APPL_RESPONSE_ERROR:
                hasApplResponseError = true;
                applResponseError = getInt(buffer, valueOffset, endOfField, 1354, CODEC_VALIDATION_ENABLED);
                break;
     ...
     * {@inheritDoc}
     */
    public ApplIDRequestAckGrpEncoder.ApplIDsGroupEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((ApplIDRequestAckGrpEncoder.ApplIDsGroupEncoder)encoder);
    }

    public ApplIDRequestAckGrpEncoder.ApplIDsGroupEncoder toEncoder(final ApplIDRequestAckGrpEncoder.ApplIDsGroupEncoder encoder)
    {
        encoder.reset();
        if (hasRefApplID())
        {
    ...
    {
        private final ApplIDRequestAckGrpDecoder parent;
        private int remainder;
        private ApplIDsGroupDecoder current;

        public ApplIDsGroupIterator(final ApplIDRequestAckGrpDecoder parent)
        {
            this.parent = parent;
        }
