...
public interface StrmAsgnRptInstrmtGrpDecoder 
{
...
public class RelatedSymGroupDecoder extends CommonDecoderImpl implements InstrumentDecoder
{
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            GROUP_FIELDS.add(Constants.STREAM_ASGN_TYPE);
            GROUP_FIELDS.add(Constants.M_D_STREAM_ID);
            GROUP_FIELDS.add(Constants.STREAM_ASGN_REJ_REASON);
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            ALL_GROUP_FIELDS.add(Constants.STREAM_ASGN_TYPE);
            ALL_GROUP_FIELDS.add(Constants.M_D_STREAM_ID);
            ALL_GROUP_FIELDS.add(Constants.STREAM_ASGN_REJ_REASON);
    ...
    public boolean validate()
    {
        ...
        {
        if (CODEC_REJECT_UNKNOWN_ENUM_VALUE_ENABLED && !StreamAsgnType.isValid(streamAsgnType()))
        {
            invalidTagId = 1617;
            rejectReason = 5;
            return false;
        ...
        {
        if (CODEC_REJECT_UNKNOWN_ENUM_VALUE_ENABLED && !StreamAsgnRejReason.isValid(streamAsgnRejReason()))
        {
            invalidTagId = 1502;
            rejectReason = 5;
            return false;
 ...
 return SettlType.NULL_VAL;
    ...
    private int streamAsgnType = MISSING_INT;
    ...
    public int streamAsgnType()
    {
        if (!hasStreamAsgnType)
        {
            throw new IllegalArgumentException("No value for optional field: StreamAsgnType");
        }
    ...
    private final CharArrayWrapper streamAsgnTypeWrapper = new CharArrayWrapper();
    public StreamAsgnType streamAsgnTypeAsEnum()
    {
        if (!hasStreamAsgnType)
 return StreamAsgnType.NULL_VAL;
        return StreamAsgnType.decode(streamAsgnType);
    }
    ...
    public int streamAsgnRejReason()
    {
        if (!hasStreamAsgnRejReason)
        {
            throw new IllegalArgumentException("No value for optional field: StreamAsgnRejReason");
        }
    ...
    private final CharArrayWrapper streamAsgnRejReasonWrapper = new CharArrayWrapper();
    public StreamAsgnRejReason streamAsgnRejReasonAsEnum()
    {
        if (!hasStreamAsgnRejReason)
 return StreamAsgnRejReason.NULL_VAL;
        return StreamAsgnRejReason.decode(streamAsgnRejReason);
    }
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.STREAM_ASGN_TYPE:
                hasStreamAsgnType = true;
                streamAsgnType = getInt(buffer, valueOffset, endOfField, 1617, CODEC_VALIDATION_ENABLED);
                break;
            ...
            case Constants.STREAM_ASGN_REJ_REASON:
                hasStreamAsgnRejReason = true;
                streamAsgnRejReason = getInt(buffer, valueOffset, endOfField, 1502, CODEC_VALIDATION_ENABLED);
                break;
     ...
     * {@inheritDoc}
     */
    public StrmAsgnRptInstrmtGrpEncoder.RelatedSymGroupEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((StrmAsgnRptInstrmtGrpEncoder.RelatedSymGroupEncoder)encoder);
    }

    public StrmAsgnRptInstrmtGrpEncoder.RelatedSymGroupEncoder toEncoder(final StrmAsgnRptInstrmtGrpEncoder.RelatedSymGroupEncoder encoder)
    {
        encoder.reset();
    ...
    {
        private final StrmAsgnRptInstrmtGrpDecoder parent;
        private int remainder;
        private RelatedSymGroupDecoder current;

        public RelatedSymGroupIterator(final StrmAsgnRptInstrmtGrpDecoder parent)
        {
            this.parent = parent;
        }
