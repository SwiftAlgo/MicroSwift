...
import com.swiftalgo.ms.codecs.fix50sp2.builder.PositionReportEncoder;
...
public class PositionReportDecoder extends CommonDecoderImpl implements ApplicationSequenceControlDecoder, PartiesDecoder, InstrumentDecoder, InstrmtLegGrpDecoder, PosUndInstrmtGrpDecoder, PositionQtyDecoder, PositionAmountDataDecoder, MessageDecoder
{
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            GROUP_FIELDS.add(Constants.MODEL_TYPE);
    ...
    public boolean validate()
    {
        ...
        if (hasModelType)
        {
        if (CODEC_REJECT_UNKNOWN_ENUM_VALUE_ENABLED && !ModelType.isValid(modelType()))
        {
            invalidTagId = 1434;
            rejectReason = 5;
            return false;
    ...
    public final IntHashSet messageFields = new IntHashSet(700);

    {
        ...
        messageFields.add(Constants.UNDERLYING_DELIVERY_AMOUNT);
        messageFields.add(Constants.NO_UNDERLYING_AMOUNTS_GROUP_COUNTER);
        messageFields.add(Constants.UNDERLYING_PAY_AMOUNT);
        messageFields.add(Constants.UNDERLYING_COLLECT_AMOUNT);
        messageFields.add(Constants.UNDERLYING_SETTLEMENT_DATE);
        messageFields.add(Constants.UNDERLYING_SETTLEMENT_STATUS);
        ...
        messageFields.add(Constants.MODEL_TYPE);
    ...
    public void posReqID(final AsciiSequenceView view)
    {
        if (!hasPosReqID)
        {
            throw new IllegalArgumentException("No value for optional field: PosReqID");
        ...
        view.wrap(buffer, posReqIDOffset, posReqIDLength);
    }
 ...
 return SubscriptionRequestType.NULL_VAL;
    ...
    public int posReqResult()
    {
        if (!hasPosReqResult)
        {
            throw new IllegalArgumentException("No value for optional field: PosReqResult");
        }
 ...
 return CPProgram.NULL_VAL;
    ...
    public int settlPriceType()
    {
        if (!hasSettlPriceType)
        {
            throw new IllegalArgumentException("No value for optional field: SettlPriceType");
        }
    ...
    {
        if (!hasSettlPriceType)
 return SettlPriceType.NULL_VAL;
 ...
 return RegistStatus.NULL_VAL;
    ...
    public void deliveryDate(final AsciiSequenceView view)
    {
        if (!hasDeliveryDate)
        {
            throw new IllegalArgumentException("No value for optional field: DeliveryDate");
        ...
        view.wrap(buffer, deliveryDateOffset, deliveryDateLength);
    }


    private int modelType = MISSING_INT;
    ...
    public int modelType()
    {
        if (!hasModelType)
        {
            throw new IllegalArgumentException("No value for optional field: ModelType");
        }
    ...
    private final CharArrayWrapper modelTypeWrapper = new CharArrayWrapper();
    public ModelType modelTypeAsEnum()
    {
        if (!hasModelType)
 return ModelType.NULL_VAL;
        return ModelType.decode(modelType);
    }
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.MODEL_TYPE:
                hasModelType = true;
                modelType = getInt(buffer, valueOffset, endOfField, 1434, CODEC_VALIDATION_ENABLED);
                break;
    ...
    public PositionReportEncoder toEncoder(final PositionReportEncoder encoder)
    {
        ...
        final PosUndInstrmtGrpEncoder posUndInstrmtGrp = encoder.posUndInstrmtGrp();        if (hasNoUnderlyingsGroupCounter)
        {
            final int size = this.noUnderlyingsGroupCounter;
            UnderlyingsGroupDecoder underlyingsGroup = this.underlyingsGroup;
            UnderlyingsGroupEncoder underlyingsGroupEncoder = posUndInstrmtGrp.underlyingsGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (underlyingsGroup != null)
                {
