...
import com.swiftalgo.ms.codecs.fix50sp2.builder.QuoteRequestEncoder;
...
public class QuoteRequestDecoder extends CommonDecoderImpl implements RootPartiesDecoder, QuotReqGrpDecoder, MessageDecoder
{
    ...
    public final IntHashSet messageFields = new IntHashSet(794);
 ...
 return RespondentType.NULL_VAL;
    ...
    public QuoteRequestEncoder toEncoder(final QuoteRequestEncoder encoder)
    {
        ...
        final QuotReqGrpEncoder quotReqGrp = encoder.quotReqGrp();        if (hasNoRelatedSymGroupCounter)
        {
            final int size = this.noRelatedSymGroupCounter;
            RelatedSymGroupDecoder relatedSymGroup = this.relatedSymGroup;
            RelatedSymGroupEncoder relatedSymGroupEncoder = quotReqGrp.relatedSymGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (relatedSymGroup != null)
                {
