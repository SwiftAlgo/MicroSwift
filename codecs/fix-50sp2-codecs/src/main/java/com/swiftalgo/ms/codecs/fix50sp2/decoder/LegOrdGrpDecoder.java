...
import com.swiftalgo.ms.codecs.fix50sp2.builder.LegOrdGrpEncoder;
import static com.swiftalgo.ms.codecs.fix50sp2.builder.LegOrdGrpEncoder.LegsGroupEncoder;
...
public interface LegOrdGrpDecoder 
{



public class LegsGroupDecoder extends CommonDecoderImpl implements InstrumentLegDecoder, LegStipulationsDecoder, LegPreAllocGrpDecoder, NestedPartiesDecoder
{
    ...
    public final IntHashSet GROUP_FIELDS = new IntHashSet(134);

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            GROUP_FIELDS.add(Constants.LEG_ALLOC_ID);
            ...
            GROUP_FIELDS.add(Constants.LEG_ORDER_QTY);
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            ALL_GROUP_FIELDS.add(Constants.LEG_ALLOC_ID);
            ...
            ALL_GROUP_FIELDS.add(Constants.LEG_ORDER_QTY);
    ...
    private IntHashSet seenFields = new IntHashSet(36);
 ...
 return LegSwapType.NULL_VAL;
    ...
    private char[] legAllocID = new char[1];

    private boolean hasLegAllocID;

    public char[] legAllocID()
    {
        if (!hasLegAllocID)
        {
            throw new IllegalArgumentException("No value for optional field: LegAllocID");
        }
    ...
    {
        return hasLegAllocID;
    }


    private int legAllocIDOffset;

    private int legAllocIDLength;

    public int legAllocIDLength()
    {
        if (!hasLegAllocID)
        {
            throw new IllegalArgumentException("No value for optional field: LegAllocID");
        ...
        return legAllocIDLength;
    }

    public String legAllocIDAsString()
    {
        return hasLegAllocID ? new String(legAllocID, 0, legAllocIDLength) : null;
    }

    public void legAllocID(final AsciiSequenceView view)
    {
        if (!hasLegAllocID)
        {
            throw new IllegalArgumentException("No value for optional field: LegAllocID");
        ...
        view.wrap(buffer, legAllocIDOffset, legAllocIDLength);
    }


    private final CharArrayWrapper legAllocIDWrapper = new CharArrayWrapper();
    ...
    private DecimalFloat legOrderQty = DecimalFloat.newNaNValue();
    ...
    public DecimalFloat legOrderQty()
    {
        if (!hasLegOrderQty)
        {
            throw new IllegalArgumentException("No value for optional field: LegOrderQty");
        }
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.LEG_ALLOC_ID:
                hasLegAllocID = true;
                legAllocID = buffer.getChars(legAllocID, valueOffset, valueLength);
                legAllocIDOffset = valueOffset;
                legAllocIDLength = valueLength;
                break;
            ...
            case Constants.LEG_ORDER_QTY:
                hasLegOrderQty = true;
                legOrderQty = getFloat(buffer, legOrderQty, valueOffset, valueLength, 685, CODEC_VALIDATION_ENABLED);
                break;
    ...
    public StringBuilder appendTo(final StringBuilder builder, final int level)
    {
        ...
        if (hasLegAllocID())
        {
            ...
            builder.append(this.legAllocID(), 0, legAllocIDLength());
            builder.append("\",\n");
        }
     ...
     * {@inheritDoc}
     */
    public LegOrdGrpEncoder.LegsGroupEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((LegOrdGrpEncoder.LegsGroupEncoder)encoder);
    }

    public LegOrdGrpEncoder.LegsGroupEncoder toEncoder(final LegOrdGrpEncoder.LegsGroupEncoder encoder)
    {
        encoder.reset();
        ...
        if (hasLegAllocID())
        {
            encoder.legAllocID(this.legAllocID(), 0, legAllocIDLength());
        }


        final LegPreAllocGrpEncoder legPreAllocGrp = encoder.legPreAllocGrp();        if (hasNoLegAllocsGroupCounter)
        {
            final int size = this.noLegAllocsGroupCounter;
            LegAllocsGroupDecoder legAllocsGroup = this.legAllocsGroup;
            LegAllocsGroupEncoder legAllocsGroupEncoder = legPreAllocGrp.legAllocsGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (legAllocsGroup != null)
                {
    ...
    {
        private final LegOrdGrpDecoder parent;
        private int remainder;
        private LegsGroupDecoder current;

        public LegsGroupIterator(final LegOrdGrpDecoder parent)
        {
            this.parent = parent;
        }
