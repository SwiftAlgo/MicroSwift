...
import com.swiftalgo.ms.codecs.fix50sp2.builder.OrderMassActionReportEncoder;
...
public class OrderMassActionReportDecoder extends CommonDecoderImpl implements AffectedOrdGrpDecoder, NotAffectedOrdersGrpDecoder, PartiesDecoder, TargetPartiesDecoder, InstrumentDecoder, UnderlyingInstrumentDecoder, MessageDecoder
{
...

    {
        ...
        {
            REQUIRED_FIELDS.add(Constants.MASS_ACTION_REPORT_ID);
            REQUIRED_FIELDS.add(Constants.MASS_ACTION_TYPE);
            REQUIRED_FIELDS.add(Constants.MASS_ACTION_SCOPE);
            REQUIRED_FIELDS.add(Constants.MASS_ACTION_RESPONSE);
        }
    }

    public final IntHashSet GROUP_FIELDS = new IntHashSet(340);

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            GROUP_FIELDS.add(Constants.MASS_ACTION_REPORT_ID);
            GROUP_FIELDS.add(Constants.MASS_ACTION_TYPE);
            GROUP_FIELDS.add(Constants.MASS_ACTION_SCOPE);
            GROUP_FIELDS.add(Constants.MASS_ACTION_RESPONSE);
            GROUP_FIELDS.add(Constants.MASS_ACTION_REJECT_REASON);
    ...
    private final IntHashSet alreadyVisitedFields = new IntHashSet(340);
    ...
    public boolean validate()
    {
        ...
        if (CODEC_REJECT_UNKNOWN_ENUM_VALUE_ENABLED && !MassActionResponse.isValid(massActionResponse()))
        {
            invalidTagId = 1375;
            rejectReason = 5;
            return false;
        ...
        {
        if (CODEC_REJECT_UNKNOWN_ENUM_VALUE_ENABLED && !MassActionRejectReason.isValid(massActionRejectReason()))
        {
            invalidTagId = 1376;
            rejectReason = 5;
            return false;
        ...
        if (hasNoNotAffectedOrdersGroupCounter)
        {
            {
                int count = 0;
                final NotAffectedOrdersGroupIterator iterator = notAffectedOrdersGroupIterator.iterator();
                for (final NotAffectedOrdersGroupDecoder group : iterator)
                {
                    count++;                    if (!group.validate())
                    {
                        invalidTagId = group.invalidTagId();
    ...
    public static final long MESSAGE_TYPE = 23106L;

    public static final String MESSAGE_TYPE_AS_STRING = "BZ";
...

    {
        ...
        messageFields.add(Constants.MASS_ACTION_REPORT_ID);
    ...
    private int massActionReportIDOffset;

    private int massActionReportIDLength;

    public int massActionReportIDLength()
    {
        return massActionReportIDLength;
    }

    public String massActionReportIDAsString()
    {
        return new String(massActionReportID, 0, massActionReportIDLength);
    }

    public void massActionReportID(final AsciiSequenceView view)
    {
        view.wrap(buffer, massActionReportIDOffset, massActionReportIDLength);
    }
    ...
    private int massActionResponse = MISSING_INT;
    ...
    private final CharArrayWrapper massActionResponseWrapper = new CharArrayWrapper();
    public MassActionResponse massActionResponseAsEnum()
    {
        return MassActionResponse.decode(massActionResponse);
    }

    private int massActionRejectReason = MISSING_INT;
    ...
    public int massActionRejectReason()
    {
        if (!hasMassActionRejectReason)
        {
            throw new IllegalArgumentException("No value for optional field: MassActionRejectReason");
        }
    ...
    private final CharArrayWrapper massActionRejectReasonWrapper = new CharArrayWrapper();
    public MassActionRejectReason massActionRejectReasonAsEnum()
    {
        if (!hasMassActionRejectReason)
 return MassActionRejectReason.NULL_VAL;
        return MassActionRejectReason.decode(massActionRejectReason);
    }
    ...
    private NotAffectedOrdersGroupDecoder notAffectedOrdersGroup = null;
    public NotAffectedOrdersGroupDecoder notAffectedOrdersGroup()
    {
        return notAffectedOrdersGroup;
    }

    private int noNotAffectedOrdersGroupCounter = MISSING_INT;
    ...
    public int noNotAffectedOrdersGroupCounter()
    {
        if (!hasNoNotAffectedOrdersGroupCounter)
        {
            throw new IllegalArgumentException("No value for optional field: NoNotAffectedOrdersGroupCounter");
        }
    ...
    private NotAffectedOrdersGroupIterator notAffectedOrdersGroupIterator = new NotAffectedOrdersGroupIterator(this);
    public NotAffectedOrdersGroupIterator notAffectedOrdersGroupIterator()
    {
        return notAffectedOrdersGroupIterator.iterator();
    }
 ...
 return Side.NULL_VAL;
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.MASS_ACTION_REPORT_ID:
                massActionReportID = buffer.getChars(massActionReportID, valueOffset, valueLength);
                massActionReportIDOffset = valueOffset;
                massActionReportIDLength = valueLength;
                break;
            ...
            case Constants.MASS_ACTION_RESPONSE:
                massActionResponse = getInt(buffer, valueOffset, endOfField, 1375, CODEC_VALIDATION_ENABLED);
                break;

            case Constants.MASS_ACTION_REJECT_REASON:
                hasMassActionRejectReason = true;
                massActionRejectReason = getInt(buffer, valueOffset, endOfField, 1376, CODEC_VALIDATION_ENABLED);
                break;
            ...
            case Constants.NO_NOT_AFFECTED_ORDERS_GROUP_COUNTER:
                ...
                {
                    notAffectedOrdersGroup = new NotAffectedOrdersGroupDecoder(trailer, messageFields);
                }
                NotAffectedOrdersGroupDecoder notAffectedOrdersGroupCurrent = notAffectedOrdersGroup;
                position = endOfField + 1;
                final int noNotAffectedOrdersGroupCounter = this.noNotAffectedOrdersGroupCounter;
                for (int i = 0; i < noNotAffectedOrdersGroupCounter && position < end; i++)
                {
                    if (notAffectedOrdersGroupCurrent != null)
                    {
                        position += notAffectedOrdersGroupCurrent.decode(buffer, position, end - position);
                        notAffectedOrdersGroupCurrent = notAffectedOrdersGroupCurrent.next();
                    }
                }
                if (CODEC_VALIDATION_ENABLED)
                {
                    ...
                    {
                        final int checkTag = buffer.getInt(position, checkEqualsPosition);
                        if (notAffectedOrdersGroup.ALL_GROUP_FIELDS.contains(checkTag))
                        {
                            invalidTagId = tag;
                            rejectReason = 16;
                            return position;
    ...
    {
        massActionResponse = MISSING_INT;
    }
    ...
    {
        for (final NotAffectedOrdersGroupDecoder notAffectedOrdersGroupDecoder : notAffectedOrdersGroupIterator.iterator())
        {
            notAffectedOrdersGroupDecoder.reset();
            if (notAffectedOrdersGroupDecoder.next() == null)
            {
                break;
            }
        }
        noNotAffectedOrdersGroupCounter = MISSING_INT;
    ...
    public StringBuilder appendTo(final StringBuilder builder, final int level)
    {
        ...
        builder.append(this.massActionReportID(), 0, massActionReportIDLength());
        builder.append("\",\n");
     ...
     * {@inheritDoc}
     */
    public OrderMassActionReportEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((OrderMassActionReportEncoder)encoder);
    }

    public OrderMassActionReportEncoder toEncoder(final OrderMassActionReportEncoder encoder)
    {
        encoder.reset();
        if (hasClOrdID())
        {
        ...
        encoder.massActionReportID(this.massActionReportID(), 0, massActionReportIDLength());
        ...
        final AffectedOrdGrpEncoder affectedOrdGrp = encoder.affectedOrdGrp();        if (hasNoAffectedOrdersGroupCounter)
        {
            final int size = this.noAffectedOrdersGroupCounter;
            AffectedOrdersGroupDecoder affectedOrdersGroup = this.affectedOrdersGroup;
            AffectedOrdersGroupEncoder affectedOrdersGroupEncoder = affectedOrdGrp.affectedOrdersGroup(size);
        ...
        final NotAffectedOrdersGrpEncoder notAffectedOrdersGrp = encoder.notAffectedOrdersGrp();        if (hasNoNotAffectedOrdersGroupCounter)
        {
            final int size = this.noNotAffectedOrdersGroupCounter;
            NotAffectedOrdersGroupDecoder notAffectedOrdersGroup = this.notAffectedOrdersGroup;
            NotAffectedOrdersGroupEncoder notAffectedOrdersGroupEncoder = notAffectedOrdersGrp.notAffectedOrdersGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (notAffectedOrdersGroup != null)
                {
                    notAffectedOrdersGroup.toEncoder(notAffectedOrdersGroupEncoder);
                    notAffectedOrdersGroup = notAffectedOrdersGroup.next();
