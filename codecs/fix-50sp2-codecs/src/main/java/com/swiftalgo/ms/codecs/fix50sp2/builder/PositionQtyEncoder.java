...
public static class PositionsGroupEncoder
{
    ...
    private static final int quantityDateHeaderLength = 4;
    private static final byte[] quantityDateHeader = new byte[] {57, 55, 54, (byte) '='};
    ...
    private final MutableDirectBuffer quantityDate = new UnsafeBuffer();

    private int quantityDateOffset = 0;

    private int quantityDateLength = 0;

    public PositionsGroupEncoder quantityDate(final DirectBuffer value, final int offset, final int length)
    {
        quantityDate.wrap(value);
        quantityDateOffset = offset;
        quantityDateLength = length;
        return this;
    }

    public PositionsGroupEncoder quantityDate(final DirectBuffer value, final int length)
    {
        return quantityDate(value, 0, length);
    }

    public PositionsGroupEncoder quantityDate(final DirectBuffer value)
    {
        return quantityDate(value, 0, value.capacity());
    }

    public PositionsGroupEncoder quantityDate(final byte[] value, final int offset, final int length)
    {
        quantityDate.wrap(value);
        quantityDateOffset = offset;
        quantityDateLength = length;
    ...
    public PositionsGroupEncoder quantityDateAsCopy(final byte[] value, final int offset, final int length)
    {
        copyInto(quantityDate, value, offset, length);
        quantityDateOffset = offset;
        quantityDateLength = length;
        return this;
    }

    public PositionsGroupEncoder quantityDate(final byte[] value, final int length)
    {
        return quantityDate(value, 0, length);
    }

    public PositionsGroupEncoder quantityDate(final byte[] value)
    {
        return quantityDate(value, 0, value.length);
    }

    public boolean hasQuantityDate()
    {
        return quantityDateLength > 0;
    }

    public MutableDirectBuffer quantityDate()
    {
        return quantityDate;
    }
    ...
    {
        return quantityDate.getStringWithoutLengthAscii(quantityDateOffset, quantityDateLength);
    }
    ...
    public int encode(final MutableAsciiBuffer buffer, final int offset, final int remainingElements)
    {
        ...
        if (quantityDateLength > 0)
        {
            buffer.putBytes(position, quantityDateHeader, 0, quantityDateHeaderLength);
            position += quantityDateHeaderLength;
            buffer.putBytes(position, quantityDate, quantityDateOffset, quantityDateLength);
            position += quantityDateLength;
            buffer.putSeparator(position);
            position++;
        }
    ...
    public StringBuilder appendTo(final StringBuilder builder, final int level)
    {
        ...
        if (hasQuantityDate())
        {
            ...
            appendBuffer(builder, quantityDate, quantityDateOffset, quantityDateLength);
            builder.append("\",\n");
        }
    ...
    public PositionsGroupEncoder copyTo(final PositionsGroupEncoder encoder)
    {
        ...
        if (hasQuantityDate())
        {
            encoder.quantityDateAsCopy(quantityDate.byteArray(), 0, quantityDateLength);
        }
