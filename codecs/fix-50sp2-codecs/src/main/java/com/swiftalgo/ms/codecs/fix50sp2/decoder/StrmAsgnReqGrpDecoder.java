...
public interface StrmAsgnReqGrpDecoder 
{



public class AsgnReqsGroupDecoder extends CommonDecoderImpl implements PartiesDecoder, StrmAsgnReqInstrmtGrpDecoder
{
    ...
    public final IntHashSet GROUP_FIELDS = new IntHashSet(0);
    ...
    public AsgnReqsGroupDecoder(final TrailerDecoder trailer, final IntHashSet messageFields)
    {
        this.trailer = trailer;
        this.messageFields = messageFields;
    }

    private AsgnReqsGroupDecoder next = null;
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            if (!seenFields.add(tag))
            {
                if (next == null)
                {
                    next = new AsgnReqsGroupDecoder(trailer, messageFields);
                }
    ...
    public StrmAsgnReqGrpEncoder.AsgnReqsGroupEncoder toEncoder(final StrmAsgnReqGrpEncoder.AsgnReqsGroupEncoder encoder)
    {
        ...
        final StrmAsgnReqInstrmtGrpEncoder strmAsgnReqInstrmtGrp = encoder.strmAsgnReqInstrmtGrp();        if (hasNoRelatedSymGroupCounter)
        {
            final int size = this.noRelatedSymGroupCounter;
            RelatedSymGroupDecoder relatedSymGroup = this.relatedSymGroup;
            RelatedSymGroupEncoder relatedSymGroupEncoder = strmAsgnReqInstrmtGrp.relatedSymGroup(size);
            for (int i = 0; i < size; i++)
            {
                if (relatedSymGroup != null)
                {
    ...
    public class AsgnReqsGroupIterator implements Iterable<AsgnReqsGroupDecoder>, java.util.Iterator<AsgnReqsGroupDecoder>
    {
        private final StrmAsgnReqGrpDecoder parent;
        private int remainder;
        private AsgnReqsGroupDecoder current;

        public AsgnReqsGroupIterator(final StrmAsgnReqGrpDecoder parent)
        {
            this.parent = parent;
        }
        ...
        {
            remainder--;
            final AsgnReqsGroupDecoder value = current;
