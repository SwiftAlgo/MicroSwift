...
import com.swiftalgo.ms.codecs.fix50sp2.builder.StreamAssignmentRequestEncoder;
import com.swiftalgo.ms.codecs.fix50sp2.builder.StrmAsgnReqGrpEncoder;
import static com.swiftalgo.ms.codecs.fix50sp2.builder.StrmAsgnReqGrpEncoder.AsgnReqsGroupEncoder;
...
public class StreamAssignmentRequestDecoder extends CommonDecoderImpl implements StrmAsgnReqGrpDecoder, MessageDecoder
{
...

    {
        ...
        {
            REQUIRED_FIELDS.add(Constants.STREAM_ASGN_REQ_ID);
            REQUIRED_FIELDS.add(Constants.STREAM_ASGN_REQ_TYPE);
        }
    }
    ...
    public static final long MESSAGE_TYPE = 17219L;

    public static final String MESSAGE_TYPE_AS_STRING = "CC";

    public static final char[] MESSAGE_TYPE_CHARS = MESSAGE_TYPE_AS_STRING.toCharArray();

    public static final byte[] MESSAGE_TYPE_BYTES = MESSAGE_TYPE_AS_STRING.getBytes(US_ASCII);

    public final IntHashSet messageFields = new IntHashSet(320);
    ...
    {
        return new String(streamAsgnReqID, 0, streamAsgnReqIDLength);
    }
    ...
    {
        // Decode StreamAssignmentRequest
     ...
     * {@inheritDoc}
     */
    public StreamAssignmentRequestEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((StreamAssignmentRequestEncoder)encoder);
    }

    public StreamAssignmentRequestEncoder toEncoder(final StreamAssignmentRequestEncoder encoder)
    {
        encoder.reset();
        encoder.streamAsgnReqID(this.streamAsgnReqID(), 0, streamAsgnReqIDLength());
        encoder.streamAsgnReqType(this.streamAsgnReqType());

        final StrmAsgnReqGrpEncoder strmAsgnReqGrp = encoder.strmAsgnReqGrp();        if (hasNoAsgnReqsGroupCounter)
        {
            final int size = this.noAsgnReqsGroupCounter;
            AsgnReqsGroupDecoder asgnReqsGroup = this.asgnReqsGroup;
            AsgnReqsGroupEncoder asgnReqsGroupEncoder = strmAsgnReqGrp.asgnReqsGroup(size);
