...
public interface QuotEntryAckGrpDecoder 
{



public class QuoteEntriesGroupDecoder extends CommonDecoderImpl implements InstrumentDecoder, InstrmtLegGrpDecoder
{
    ...
    public final IntHashSet GROUP_FIELDS = new IntHashSet(226);

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            GROUP_FIELDS.add(Constants.QUOTE_ENTRY_STATUS);
...

    {
        if (CODEC_VALIDATION_ENABLED)
        {
            ...
            ALL_GROUP_FIELDS.add(Constants.QUOTE_ENTRY_STATUS);
    ...
    public boolean validate()
    {
        ...
        {
        if (CODEC_REJECT_UNKNOWN_ENUM_VALUE_ENABLED && !QuoteEntryStatus.isValid(quoteEntryStatus()))
        {
            invalidTagId = 1167;
            rejectReason = 5;
            return false;
 ...
 return OrderCapacity.NULL_VAL;
    ...
    private int quoteEntryStatus = MISSING_INT;

    private boolean hasQuoteEntryStatus;

    public int quoteEntryStatus()
    {
        if (!hasQuoteEntryStatus)
        {
            throw new IllegalArgumentException("No value for optional field: QuoteEntryStatus");
        }

        return quoteEntryStatus;
    }

    public boolean hasQuoteEntryStatus()
    {
        return hasQuoteEntryStatus;
    }



    private final CharArrayWrapper quoteEntryStatusWrapper = new CharArrayWrapper();
    public QuoteEntryStatus quoteEntryStatusAsEnum()
    {
        if (!hasQuoteEntryStatus)
 return QuoteEntryStatus.NULL_VAL;
        return QuoteEntryStatus.decode(quoteEntryStatus);
    }
    ...
    public int decode(final AsciiBuffer buffer, final int offset, final int length)
    {
        ...
        while (position < end)
        {
            ...
            case Constants.QUOTE_ENTRY_STATUS:
                hasQuoteEntryStatus = true;
                quoteEntryStatus = getInt(buffer, valueOffset, endOfField, 1167, CODEC_VALIDATION_ENABLED);
                break;
    ...
    public void resetQuoteEntryStatus()
    {
        hasQuoteEntryStatus = false;
    }
    ...
    public StringBuilder appendTo(final StringBuilder builder, final int level)
    {
        ...
        {
            indent(builder, level);
            builder.append("\"QuoteEntryStatus\": \"");
            builder.append(quoteEntryStatus);
            builder.append("\",\n");
        }
     ...
     * {@inheritDoc}
     */
    public QuotEntryAckGrpEncoder.QuoteEntriesGroupEncoder toEncoder(final Encoder encoder)
    {
        return toEncoder((QuotEntryAckGrpEncoder.QuoteEntriesGroupEncoder)encoder);
    }

    public QuotEntryAckGrpEncoder.QuoteEntriesGroupEncoder toEncoder(final QuotEntryAckGrpEncoder.QuoteEntriesGroupEncoder encoder)
    {
        encoder.reset();
        if (hasQuoteEntryID())
        {
        ...
        if (hasQuoteEntryStatus())
        {
            encoder.quoteEntryStatus(this.quoteEntryStatus());
        }
    ...
    {
        private final QuotEntryAckGrpDecoder parent;
        private int remainder;
        private QuoteEntriesGroupDecoder current;

        public QuoteEntriesGroupIterator(final QuotEntryAckGrpDecoder parent)
        {
            this.parent = parent;
        }
